 ...
 *   ?action=saveADeal
 *   ?action=tasks.today
 *   ?action=notes.add&text=Hello&tags=test&id=ID%20-%201234
 *   ?action=notes.search&q=test
 *   ?action=calendar.create&title=Delivery&start=2025-08-12T15:00:00-06:00&end=2025-08-12T16:00:00-06:00
 ******************************************************/
...
// Calendar: SPAC
const SPAC_CALENDAR_ID =
  '5d07e758da607524452232adab8c92fd7012ac98f166319de823de62b2e1fa40@group.calendar.google.com';

// Lead Log columns (1-based) — RENAMED to avoid collisions with other files in this project
const DLX_COL = {
...
// Event types that count as a "touch"
...
function doGet(e) {
  ...
  try {
    switch (action) {
      case 'lead.overview': {
        const res = leadOverview_({ id: p.id, phone: p.phone, email: p.email, name: p.name, q: p.q });
        return json_(ok(res));
      }
      case 'notes.add': {
      ...
      case 'calendar.create': {
        ...
        const guests      = (p.guests || '').split(',').map(s => s.trim()).filter(Boolean);
...
function saveADeal_() {
  ...
  for (const rec of rows) {
    ...
    out.push({ lead, tasks, waitingOn, lastTouch: comms.length ? comms[0] : null });
  }
...
function tasksToday_() {
  ...
  for (const lead of leads) {
    ...
    // “Due today” heuristic: contains “today” or has no explicit date pattern
    ...
    if (dueToday.length) tasks.push({ lead, tasks: dueToday });
  }
...
function extractTasks_(texts) {
  ...
  for (const blob of texts) {
    ...
    for (const ln of lines) {
      const t = ln.trim();
      if (!t) continue;
      if (TASK_MARKERS.test(t) || /^\s*[-*]\s*\[?\s*\]?\s+/i.test(t)) out.push(t);
    }
  }
  return dedupe_(out);
}
...
function findCalendarEventsRelatedToLead_(lead, lookaheadDays) {
  const cal = spacCalendar_(); if (!cal) return [];
  const now = new Date();
...
function createSpacEvent_({ title, start, end, description, location, guests }) {
  ...
  return { id: event.getId(), title: event.getTitle(), start: event.getStartTime(), end: event.getEndTime(), htmlLink: event.getHtmlLink() };
}
...
function inferCOL_(sheet) {
  // Use this file’s constants if present (renamed to DLX_COL to avoid collisions)
  if (typeof DLX_COL !== 'undefined' && DLX_COL && DLX_COL.ID) return DLX_COL;
